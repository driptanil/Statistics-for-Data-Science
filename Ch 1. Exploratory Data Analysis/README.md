...
-   _Mean:_ sum of all values divided by number of values
...
This removes 10 % of data in Gold column top and bottom before calculation of mean.

_Robust_: A robust metric is one which is not sensitive to outliers
...
Median is a better example of the average medal won than mean.

-   _Weighted Median: Same as median except you multiply every value of some_ $x_i$ and pick the middle value, such that the sum of value of left of the median equals right of the median
...
-   _Percentile_: The value such that $p$ percent of the data lies below it
...
-   _Deviations_: The difference between observed values and the estimate of location, as called Errors or Residuals.
...
-   _Percentile_: The value which is greatest than $P$ percent of values in the dataset
...
<aside> 💡 The variance, standard deviation, mean absolute deviation, and median absolute deviation are not equivalent estimates.
...
Another method of measuring dispersion is looking at the spread of sorted data. Statistics based on sorted data are called order statistics. The Range is the most common of these statistics and is simply the maximum value minus the minimum value. This is incredibly sensitive to outliers so avoid this we typically take statistics that ignore the highest and lowest percentile values. The Interquartile Range is the most famous of these statistics and is the 75th percentile minus 25th percentile.

For vey large datasets, it can get computationally intensive to calculate these statistics because it requires sorted the data, so a Machine Learning algorithm might estimate these statistics

# Exploring the Data Distribution

### Percentiles and Boxplots

Percentiles are a great way to summarize the tails of a distribution such as the top 1%. You can also construct box plots which are a tremendous way to visually summarize the distribution of data in a dataset.
...
-   _Variability (2nd moment) :_ 
	  How closely value are spread around the mean (or another measure of central tendency)
	  
-   _Skewness (3rd moment)_ : 
	  Direction of the tail of the data → Discovered through visualization not a metric
	  
-   _Kurtosis (4th moment)_ : 
	  Propensity for data to have extreme

# Density Plot and Estimates

- A Density plot is a smoothed histogram and shows the distribution of data values as a continuous line, and the y-axis is plotted as proportion. 
- The proportion part is the key difference here, and the reason that density plots are not the same as histograms.
...
- *Bar Charts*: Plots each variable agianst it's frequency or proportion
...
- *Pie Charts:*  Plots the grequency of each vategoru as wedges of a pie (generally not a recommended chart type as the size of each pie can be difficult to compare)
...
Probabilities are another way to describe categorical or binary data by grouping each variable with its likelihood of being chosen (proportional of the entire dataset)
...
$$
- *Correlation coefficient* : Generally use Pearson's Correlation Coefficient. The PCC Is sensitive to outliers. This measures how change in one variable will change another variable.
...
![](hexagonalBinning.png)

- *Contour Plot*: 
	  Plots showing density of two numeric variables, similar to a topographical map  
...
- *Scatterplot Plot:*
	- Plots one numerical variable on one axis vs antoher on a different axis, with each point correlating to a record. 
	  
	- Scatterplots offer an excellent way to measure the relationshop between two variables.
...
- *Heat Maps:* 
	  Heat maps offer another way to look at data by using colors to communicate densities. Heatmaps cal also be used with categorical variables 
